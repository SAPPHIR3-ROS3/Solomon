...
- **Explicit surfaces** — CLI modes (`exec`, `temp exec`, `add`/`remove`), REPL slash commands, separate plan vs build tooling.
...
| `internal/tooling/` | Legacy/native tool invocation parse |
...
| System prompts | Templates in `internal/prompt/templates/` via `RenderPlan` / `RenderBuild` |
