...
func getSlashBuiltins() []slashBuiltin {
	...
	slashBuiltins = []slashBuiltin{
		...
		{[]string{"connect"}, "/connect", "connect ChatGPT Sub, OpenAI-compatible API, Anthropic API key, Claude Sub, Cursor API, or Cursor Sub; then pick model", nil, func(d Deps, parts []string) error { return Connect(d) }},
		...
		{[]string{"add"}, "/add", "/add rule <phrase> | /add projectrule <phrase> | /add subagent | skills.sh URL | npx skills add ... | skill <.md> [name] [global|project|local] — scope default global", nil, func(d Deps, parts []string) error {
			if len(parts) < 2 {
				return fmt.Errorf(`usage: /add rule <phrase> | /add projectrule <phrase> | /add subagent | npx ... | skills.sh | skill <.md> [name] [scope]`)
			}
