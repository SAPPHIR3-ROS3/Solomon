...
func getSlashBuiltins() []slashBuiltin {
	...
	slashBuiltins = []slashBuiltin{
		...
		{[]string{"connect"}, "/connect", "connect ChatGPT Sub (OAuth), OpenAI-compatible API, or Anthropic Compatible API; then pick model", func(d Deps, parts []string) error { return Connect(d) }},
