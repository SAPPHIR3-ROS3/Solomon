...
func connectChooseKind(d Deps, sc *bufio.Scanner) (int, error) {
	...
	case "2":
		return 2, nil
	default:
		return 0, fmt.Errorf("invalid selection %q (use 1 or 2)", line)
	}
}
...
func connectCompatibleAPI(d Deps, sc *bufio.Scanner) error {
	...
	prov := config.Provider{Name: n, BaseURL: base, APIKey: k, AuthKind: config.AuthKindAPIKey}
	ctx := d.Ctx
	if ctx == nil {
		ctx = context.Background()
	}
