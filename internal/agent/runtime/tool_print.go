...
const nativeBridgeToolCorrectionUserMsg = "Your previous reply did not include valid native API tool_calls. Emit native Solomon tools only (orchestrate, searchTools, subagent, switchMode, searchSkill, loadSkill) via API tool_calls with JSON arguments that match each tool schema, including a non-empty intent string — not <tool_calls> XML or plain-text tool narration. For workspace and MCP work, call searchTools if unsure, then orchestrate (package main, import \"sdk\"), using sdk.mcp.<tool>(intent, args) for connected MCP tools — never emit deferred tools or MCP.* tools as direct native tool_calls. Send a corrected invocation or continue without tools if you meant plain text."
...
func redirectExtraCorrectionHint(toolName string) string {
	...
	case "callmcptool", "fetchmcpresource", "listmcpresources":
		return "MCP wrappers are unavailable: use searchTools for schemas, then orchestrate with sdk.mcp.<tool>(intent, args)."
	default:
		return ""
	}
}
