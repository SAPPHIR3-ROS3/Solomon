...
func TestParseConfigExpandsEnvAndFiltersTools(t *testing.T) {
	...
	fs := cfg.Servers[0]
	if fs.Name != "filesystem" || fs.Type != TransportStdio || fs.Args[1] != "/tmp/workspace" || fs.Env["TOKEN"] != "secret" {
	...
	if remote.Type != TransportStreamableHTTP || !strings.Contains(remote.URL, "secret") || remote.Headers["Authorization"] != "Bearer secret" || remote.Timeout != 300000 {
...
func TestParseConfigMissingEnvVar(t *testing.T) {
	t.Setenv("PRESENT", "ok")
	_, err := ParseConfig([]byte(`{"mcpServers": {"x": {"command": "$PRESENT", "args": ["$MISSING"]}}}`))
