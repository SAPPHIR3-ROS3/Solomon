...
func scoreGrok(id string) flagshipScore {
	...
	rest = strings.TrimPrefix(rest, "grok")
	ver := digitsVersionKey(rest)
	if len(ver) == 0 {
		parts := strings.Split(rest, "-")
		ver = parseVersionSegment(parts[0])
	}
	...
	return flagshipScore{ver: ver, tier: grokVariantTier(rest), ok: true}
}

func grokVariantTier(rest string) int {
	switch {
	case strings.Contains(rest, "low"):
		return 40
	case strings.Contains(rest, "medium"):
		return 60
	case strings.Contains(rest, "high"):
		return 80
