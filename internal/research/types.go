...
// cloneJobRecord returns an independent snapshot of a job record. Job records
// are updated by a background research goroutine while status consumers and
// UI callbacks read them concurrently, so returning the stored slices would
// reintroduce aliases even when the manager lock is held during the copy.
func cloneJobRecord(rec JobRecord) JobRecord {
	out := rec
	out.Findings = append([]Finding(nil), rec.Findings...)
	out.QueriesUsed = append([]string(nil), rec.QueriesUsed...)
	out.URLsFetched = append([]string(nil), rec.URLsFetched...)
	out.URLAttempts = append([]URLAttempt(nil), rec.URLAttempts...)
	return out
}
